---
title: LLM 的概率本质——为什么 AI 会"胡说"
date: 2026-04-26
---

# LLM 的概率本质——为什么 AI 会"胡说"
...
但关键区别在于：**LLM 并不"理解"它在说什么，它只是在统计意义上预测"接下来最可能出现什么词"**。[^2]
...
**SentencePiece**：直接操作原始文本的 Unicode 字节流，不依赖空格预分词。它将空格也视为一个可学习的 token（通常用 `▁` 表示），因此对多语言和噪声文本更鲁棒。T5、LLaMA 等模型采用此方案。
...
- **Temperature > 1**：分布被"flatten"（压平），低概率 token 获得更多机会，输出更随机、更有"创意"
...
| 问答、事实检索 | Temperature: 0.1-0.5, Top-p: 0.5-0.9 |
...
根本原因在于训练目标的错位。模型被训练来**最大化训练数据的似然**（即让预测尽可能接近真实文本），而不是**最大化真实性**。在训练数据中，"我不知道"出现的频率远低于直接猜测的尝试。此外，后训练阶段（如 RLHF）如果不对"错误回答"和"弃权"做差异化惩罚，模型就没有动力去承认不确定。[^5]
...
需要承认的是，随着模型规模增大，LLM 确实展现出了一些令人惊讶的能力——链式思考（Chain-of-Thought）、少样本学习（Few-Shot Learning）、甚至某种程度的抽象概括。Anthropic 的研究团队通过**电路追踪（Circuit Tracing）**技术，发现模型内部确实存在可解释的概念表征（如"大小"、"相反"等概念的特征激活）。[^8]
...
并非所有研究者都认同"涌现"是一个真实的、不可预测的现象。一些学者认为，所谓的"涌现"可能只是评估指标的非线性变化所致——如果改用连续的评估指标，能力的"突变"就会消失。但无论如何，**大模型确实展现出了小模型不具备的复杂行为**，这一点是共识。[^9]
...
根据 Comet 等团队的实践经验，以下场景必须引入人工审查（Human-in-the-Loop）：[^11]
...
[^1]: wwwinsights.com, "How Large Language Models Really Work: Next-Token Prediction at the Core", 2025. https://www.wwwinsights.com/ai/llm-next-token-prediction/

[^2]: faridkazimov.com, "How Do LLMs Work? Understanding Next Token Prediction", 2026. https://faridkazimov.com/blog/2026/how-llms-work/
...
[^4]: Machine Learning Plus, "LLM Temperature, Top-P, and Top-K Explained — With Python Simulations", 2025. https://machinelearningplus.com/gen-ai/llm-temperature-top-p-top-k-explained/

[^5]: OpenAI, "Why Language Models Hallucinate", 2025. https://openai.com/index/why-language-models-hallucinate/ (论文 arXiv:2509.04664)

[^6]: Arize AI, "OpenAI's Santosh Vempala Explains Why Language Models Hallucinate", 2025. https://arize.com/blog/openais-santosh-vempala-explains-why-language-models-hallucinate/
...
[^9]: Wei, J., et al. "Emergent Abilities of Large Language Models." arXiv:2206.07682, Google Research, 2022. https://arxiv.org/abs/2206.07682

[^10]: Vaswani, A., et al. "Attention Is All You Need." NeurIPS 2017. (Transformer 架构原始论文)

[^11]: Comet, "Human-in-the-Loop Review Workflows for LLM Applications & Agents", 2025. https://www.comet.com/site/blog/human-in-the-loop/
