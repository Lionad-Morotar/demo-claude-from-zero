...
Claude Code 的心脏是一个极简的 `while(tool_call)` 循环，内部代号 **nO**。其伪代码如下：
...
while (true) {
  ...
  // 2. 调用 Claude 模型进行推理
  const response = await claudeAPI.complete(state);
...
值得注意的是，Anthropic 早期版本曾实验使用 Voyage 嵌入进行语义代码搜索（RAG），但最终切换回了基于 ripgrep 的代理式搜索。原因很务实：Claude 已经深刻理解代码结构，能够构造复杂的正则表达式，维护向量索引的复杂性和开销并不值得。
...
> "System prompts, tool definitions, MCP schemas, and memory files consume 30,000-40,000 tokens before you type anything. Performance starts degrading around 147K tokens, not 200K."[^3]
...
Claude Code 的安全不是单层防护，而是一个**五层权限门控系统**，每个工具调用都要经过完整的"五道关卡"：
...
利用 Anthropic API 的 `cache_edits` 能力，直接在服务端缓存中删除旧的工具结果，**不修改本地消息**。这是最高效的压缩方式——零成本、零信息损失（结果仍在磁盘上可读取）。
...
当用户离开超过 60 分钟（Anthropic 缓存 TTL），服务端缓存已过期。此时清理旧工具结果的代价极低——因为整个前缀都需要重写。系统将保留最近的 N 个工具结果，其余替换为占位符。
...
笔记模板包含：当前状态、任务规格、重要文件、工作流、错误与修正、学习总结等。后台提取进程作为 forked agent 运行，与主会话共享 prompt cache prefix。
...
当 Session Memory 不可用或压缩后仍超阈值时，系统回退到完整的 LLM 总结。这是通过 **forked agent** 完成的，核心目的是共享主会话的 prompt cache prefix——这样大部分输入命中现有缓存，只需为压缩指令和输出付费。
...
让我们追踪一个真实的 Claude Code 会话：用户要求"修复 auth 模块中过期卡片用户的结账流程 bug"。
...
2026 年 3 月 31 日，Anthropic 意外通过 npm registry 的 source map 泄露了 Claude Code 的完整 TypeScript 源码（约 1,900 个文件、512,000 行代码）。社区对源码的分析揭示了以下关键实现细节：
...
源码泄露揭示了 44 个未发布的功能标志，包括：

- Agent Teams（多代理协作系统，基于 tmux）
...
[^6]: VILA Lab et al. "Dive into Claude Code: The Design Space of Today's and Future AI Agent Systems." arXiv:2604.14228, 2026. https://arxiv.org/html/2604.14228v1
