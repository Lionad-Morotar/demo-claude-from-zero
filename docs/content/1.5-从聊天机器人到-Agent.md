...
# 从聊天机器人到 Agent——Planning + Memory + Tool Use
...
IBM 在一篇关于 AI Agent 与 AI Assistant 的对比文章中，用一个精妙的类比阐明了这一区别：想象你是一位明星，你的助理（Assistant）按你的要求做事——预订餐厅、整理邮件、维护日程；而你的经纪人（Agent）则主动为你规划事业、争取机会、最大化你的收入，甚至在你没有明确要求时就采取了行动。[^1]
...
OpenAI 研究员、前 AI 安全研究负责人 Lilian Weng 在 2023 年 6 月的一篇博客中，用一句简洁的公式概括了 Agent 的本质：

> **Agent = LLM + Memory + Planning Skills + Tool Use**[^2]

这个公式已经成为业界理解 Agent 架构的基准框架。Claude Code 正是这一公式的工程化落地——它以 Claude LLM 为大脑，通过工具调用与文件系统和终端交互，借助 CLAUDE.md 和 Auto Memory 实现跨会话记忆，并在每次任务中自主进行规划与执行。
...
这个循环在 Claude Code 中每天都在上演。当你要求"修复这个测试失败"时，Claude Code 的底层执行流程大致如下：
...
ReAct 论文的实验结果令人印象深刻：在问答任务 HotpotQA 和事实核查任务 FEVER 上，ReAct 克服了纯 CoT 推理中常见的幻觉和错误传播问题；在交互式决策任务 ALFWorld 和 WebShop 上，ReAct 的成功率分别比模仿学习和强化学习方法高出 34% 和 10%。[^5]

### 2.4 自我反思：从错误中学习

规划不是一次性的。真正强大的 Agent 还需要具备**自我反思（Self-Reflection）**能力——在执行过程中评估自己的表现，从错误中学习，并动态调整计划。

Shinn 和 Labash 在 2023 年提出的 Reflexion 框架，为 Agent 增加了动态记忆和自我反思能力。[^6] 在 Reflexion 中，每次行动后，Agent 会计算一个启发式评估：轨迹是否低效？是否出现了幻觉？如果发现问题，Agent 会生成反思文本，将其存入工作记忆，并在后续决策中加以利用。
...
LLM 的**上下文窗口（Context Window）**就是它的短期记忆。所有输入的提示词、历史对话、读取的文件内容，都占据这个有限的空间。当前主流模型的上下文窗口从 128K 到 2M 不等，但无论如何，它都是**有限的**。
...
Li 等人在 2023 年提出的 API-Bank 基准，将 Agent 的工具使用能力分为三个层次：[^2]
...
例如，在上述认证功能开发中，Claude Code 可能委派一个专门的安全审查子代理：
...
- **权限模式（Permission Mode）**：从只读（default）到完全自动（auto）共分五级，用户可以根据任务敏感度选择。
...
Anthropic 在《Building Effective AI Agents》一文中，将 Agentic 系统分为两类：[^12]

- **Workflows（工作流）**：LLM 和工具通过预定义的代码路径编排，适合任务明确的场景。
- **Agents（代理）**：LLM 动态指导自己的流程和工具使用，自主决定如何完成任务，适合开放式问题。

Claude Code 同时具备这两种能力：对于简单的文件编辑，它可以遵循预定义的工作流；对于复杂的重构任务，它会进入自主 Agent 模式，动态规划每一步操作。
...
[^6]: Shinn, Noah, and Labash, Beck. "Reflexion: an autonomous agent with dynamic memory and self-reflection." arXiv:2303.11366, 2023.
...
[^10]: Penligent. "Inside Claude Code, The Architecture Behind Tools, Memory, Hooks, and MCP." 2025. https://www.penligent.ai/hackinglabs/inside-claude-code-the-architecture-behind-tools-memory-hooks-and-mcp/

[^11]: Anthropic. "Model Context Protocol." 2024. https://modelcontextprotocol.io

[^12]: Schluntz, Erik, and Zhang, Barry. "Building Effective AI Agents." Anthropic Research, 2025. https://www.anthropic.com/research/building-effective-agents
