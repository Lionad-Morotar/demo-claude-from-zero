...
- [MCPpedia](https://mcppedia.org/) — MCP Server 目录（含安全评分）
...
| 作用域 | 加载范围 | 团队共享 | 存储位置 |
|--------|---------|---------|---------|
| **Project** | 仅当前项目 | 是（通过版本控制） | `.claude/settings.json` |
| **User** | 所有项目 | 否 | `~/.claude/settings.json` |

> **命令可用性声明**：以下 `claude mcp add` 语法基于社区实践。如不可用，请通过 `settings.json` 中的 `mcpServers` 字段手动配置。

```bash
# Project 作用域（团队共享，配置写入 .claude/settings.json）
claude mcp add --transport http paypal --scope project https://mcp.paypal.com/mcp

# User 作用域（跨项目，配置写入 ~/.claude/settings.json）
claude mcp add --transport http hubspot --scope user https://mcp.hubspot.com/anthropic
```

Project 作用域的配置在 `.claude/settings.json` 中示例：
...
**输出限制**：默认最大输出 25000 tokens。具体调整方式请参考官方文档。
...
| **依赖关系** | 最初由特定模型提出，现已被多平台采纳 | 模型无关（兼容 Claude/GPT/通义等） |
| **工具热插拔** | 需重新部署模型 | 工具可动态注册/卸载 |
| **跨设备调用** | 通常由宿主应用直接处理 | 支持远程/云工具调用 |
...
MCP 引入了全新的攻击面：AI 代理基于自然语言动态执行工具，访问敏感系统。以下是 MCP 使用中的关键安全风险：
...
[^1]: [Model Context Protocol 官方文档](https://modelcontextprotocol.io/) — MCP 协议规范、示例和指南的权威来源
[^2]: [MCP Specification 2025-03-26](https://modelcontextprotocol.io/specification/2025-03-26) — 协议规范的权威定义
[^3]: [MCP 中文文档](https://mcp.transdocs.org/) — 社区翻译的中文版文档
[^4]: [modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers) — 官方参考实现和社区 Server 集合
[^5]: [Claude Code MCP 文档](https://code.claude.com/docs/en/mcp) — Claude Code 中配置和使用 MCP 的完整指南
[^6]: [MCP Server Security Best Practices](https://www.truefoundry.com/blog/mcp-server-security-best-practices) — 企业级 MCP 安全部署指南
[^7]: [MCP Server 开发入门：从零搭建你的第一个 MCP 服务](https://eastondev.com/blog/zh/posts/ai/mcp-server-development-intro/) — TypeScript SDK 实战教程
[^8]: [MCP with Postgres - Querying my data in plain English](https://punits.dev/blog/mcp-with-postgres/) — PostgreSQL MCP 实战案例
[^9]: [12 MCP Server Examples Every AI Engineer Should Know](https://airbyte.com/agentic-data/mcp-server-examples) — 生产环境常用 Server 概览
[^10]: [大模型应用开发中 MCP 与 Function Call 的关系与区别](https://developer.aliyun.com/article/1695784) — 阿里云开发者社区的技术对比分析
[^11]: [MCP Server Security Best Practices](https://www.truefoundry.com/blog/mcp-server-security-best-practices) — 企业级 MCP 安全部署指南