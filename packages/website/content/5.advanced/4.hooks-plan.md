...
Hooks 是用户定义的 shell 命令、HTTP 端点、LLM 提示或子智能体，在 Claude Code 生命周期的特定节点自动执行。与 CLAUDE.md 中的指令不同——后者是"建议"，Claude 通常会遵循但不保证——Hooks 是强制执行的——事件触发时 Claude Code 会尝试执行 Hook，但 shell 脚本本身可能因环境原因失败。

> **核心原则**：CLAUDE.md 用于偏好指导（"优先使用 Bun 而非 npm"），Hooks 用于不可违背的规则（"必须用 Prettier 格式化"、"禁止触碰 .env 文件"）。

### 1.2 生命周期事件全景

截至 2026 年 3 月，Claude Code 支持至少 24 个生命周期事件，按触发频率分为三类：
...
| `UserPromptExpansion` | 用户提示扩展时 | 是 | 扩展内容审查 |
...
| `PermissionDenied` | 权限请求被拒绝时 | 是 | 同上 |
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "jq -r '.tool_input.command' < /dev/stdin | grep -qE 'rm -rf|DROP TABLE' && exit 2 || exit 0"
      }
    ],
    "PostToolUse": [
      {
...
> 某些第三方来源报告 stdout JSON 可能支持 `updatedInput` 字段以修改工具输入，但官方文档未明确证实此能力。
...
官方 Hooks 通过 stdin 接收 JSON 输入，正确读取方式为 `jq -r '.tool_input.command' < /dev/stdin`。

- `$CLAUDE_PROJECT_DIR`：项目根目录绝对路径
- `$CLAUDE_ENV_FILE`：用于 `SessionStart` 中持久化环境变量到会话（来源：第三方技术博客，官方文档待确认）
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Write|Edit|MultiEdit",
        "command": "bash -c 'FILE=$(jq -r \".tool_input.file_path\" < /dev/stdin); if [[ \"$FILE\" == *.ts || \"$FILE\" == *.tsx || \"$FILE\" == *.js || \"$FILE\" == *.jsx ]]; then npx eslint --fix \"$FILE\" 2>/dev/null; npx prettier --write \"$FILE\" 2>/dev/null; fi; exit 0'"
      }
    ]
  }
}
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Write|Edit",
        "command": "bash -c 'FILE=$(jq -r \".tool_input.file_path\" < /dev/stdin); if [[ \"$FILE\" == *.ts || \"$FILE\" == *.tsx ]]; then npx tsc --noEmit 2>&1 | head -20; fi; exit 0'",
        "timeout": 30
      }
    ]
  }
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "bash -c 'CMD=$(jq -r \".tool_input.command\" < /dev/stdin); for p in \"rm -rf /\" \"rm -rf ~\" \"drop table\" \"DROP TABLE\" \"truncate\" \"TRUNCATE\" \"--force\" \"push.*--force\"; do if echo \"$CMD\" | grep -qiE \"$p\"; then echo \"Blocked: pattern \\\"$p\\\" detected\" >&2; exit 2; fi; done; exit 0'"
      }
    ]
  }
}
...
```json
{
  "hooks": {
    "Stop": [
      {
        "command": "Review the conversation. Did the user's request get fully completed? Check: all files created, tests passing, no TODO comments left. Respond with {\"ok\": true} if done, or {\"ok\": false, \"reason\": \"what remains\"} if not.",
        "timeout": 30
      }
    ]
  }
...
```json
{
  "hooks": {
    "Stop": [
      {
        "command": "Verify the work is complete: 1) Run the test suite. 2) Check for any TypeScript errors. 3) Verify no console.log statements were left in production code. Report your findings.",
        "timeout": 120
      }
    ]
  }
...
> **注意**：`.claude.json` 路径在官方文档中未明确确认，实际配置可能使用 `.claude/settings.json`。
...
> 数据来源：Anthropic 官方工程博客 "Claude Code Auto Mode"
...
按 `Esc+Esc` 打开恢复菜单（rewind），或在命令行使用 `/rewind` 命令：
...
```json
{
  ...
  "hooks": {
    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "bash -c 'CMD=$(jq -r \".tool_input.command\" < /dev/stdin); if echo \"$CMD\" | grep -qE \"rm -rf /|rm -rf ~|DROP TABLE|force push\"; then echo \"Blocked by policy\" >&2; exit 2; fi; exit 0'"
      }
    ]
  }
}
...
```json
{
  ...
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "Evaluate whether this Bash command could affect the production environment. Consider: database operations, deployments, infrastructure changes. Respond with {\"safe\": true} or {\"safe\": false, \"reason\": \"...\"}.",
        "timeout": 10
      }
    ],
    "Stop": [
      {
        "command": "Before completing, verify: 1) No production credentials were logged. 2) No database connections were made to prod. 3) All changes are reversible. Report findings.",
        "timeout": 60
      }
    ]
  }
...
[^8]: [Claude Code Hooks Complete Guide (March 2026 Edition)](https://smartscope.blog/en/generative-ai/claude/claude-code-hooks-guide/) — 生命周期事件、处理器类型的完整指南
