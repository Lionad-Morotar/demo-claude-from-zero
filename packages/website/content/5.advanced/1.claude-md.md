...
`CLAUDE.md` 是 Claude Code 的原生项目记忆机制，一个纯 Markdown 文件，在每次会话启动时自动注入 Claude 的上下文窗口。它充当"AI 新成员的入职文档"——你写一次，Claude 在每个会话中都可访问你的项目上下文，无需重复解释。
...
研究表明，指令遵循率在约 150-200 条总指令后开始下降。Claude Code 的系统提示已占用相当一部分上下文预算，因此你大约有 100-150 个指令槽位。
...
**1. 厨房水槽文件（Kitchen Sink，指试图包含一切内容的冗长文件）**

500 行的 `CLAUDE.md` 试图记录一切。过长的 CLAUDE.md 会降低指令遵循率——建议保持精简，只包含 Claude 无法从代码推断的规则。**修复**：无情地修剪到 Claude 无法从代码推断的规则。
...
`AGENTS.md` 是社区提出的跨工具配置倡议（见 [awesome-agents-md](https://github.com/josix/awesome-claude-md)），部分工具已实验性支持。如果你的团队使用多种 AI 工具，可将其作为参考，但请注意各工具的实际支持程度可能不同。
...
[^2]: [Claude Code 官方文档 - .claude Directory](https://code.claude.com/docs/en/claude-directory)
[^3]: [Anthropic 博客 - Using CLAUDE.md files](https://code.claude.com/docs/en/memory)
[^4]: [SFEIR Institute - CLAUDE.md Memory System Tutorial](https://institute.sfeir.com/en/claude-code/claude-code-memory-system-claude-md/tutorial/)
[^5]: [Alex Rusin Blog - Claude Code Instructions and Memory Setup Guide](https://blog.alexrusin.com/claude-code-instructions-memory-setup-guide/)
[^6]: [DataCamp - Writing the Best CLAUDE.md](https://www.datacamp.com/tutorial/writing-the-best-claude-md)
[^7]: [Supalaunch - How to Write the Perfect Config File](https://supalaunch.com/blog/claude-code-claude-md-how-to-write-the-perfect-config-file-guide)
[^8]: [ChatForest - Writing Effective CLAUDE.md Files](https://chatforest.com/guides/writing-effective-claude-md-files/)
[^9]: [CodingNomads - Writing CLAUDE.md Files That Claude Actually Follows](https://codingnomads.com/claude-md-project-memory-configuration)
[^10]: [Amit Ray - Best Practices for CLAUDE.md](https://amitray.com/best-practices-for-claude-md/)
[^11]: [AI Org - Claude Code Best Practices](https://aiorg.dev/blog/claude-code-best-practices)
[^12]: [TokenCentric - AI Coding Config Files Compared](https://www.tokencentric.app/blog/ai-coding-config-files-compared)
[^13]: [AgentRuleGen - Cursor Rules vs CLAUDE.md](https://www.agentrulegen.com/guides/cursorrules-vs-claude-md)
[^14]: [Awesome CLAUDE.md (GitHub)](https://github.com/josix/awesome-claude-md)
[^15]: [Ian Paterson - Claude Code Memory Architecture](https://ianlpaterson.com/blog/claude-code-memory-architecture/)
[^16]: [CallMePhilip - Notes on CLAUDE.md Structure](https://callmephilip.com/posts/notes-on-claude-md-structure-and-best-practices/)