...
| 字段 | 必需 | 说明 |
|------|------|------|
| `name` | 是 | Skill 名称，成为 `/slash-command`。省略时使用目录名。仅限小写字母、数字和连字符，最多 64 字符 |
...
**优先级规则**：企业 > 个人 > 项目。同名 Skill 时，高优先级覆盖低优先级。
...
创建 Skill 后，通过实际对话测试触发条件和执行流程，验证输出是否符合预期。
...
Anthropic 官方 Skills 位于 `github.com/anthropics/skills` 仓库，可直接克隆到本地使用：

```bash
# 克隆官方 Skills 仓库
git clone https://github.com/anthropics/skills.git

# 将需要的 Skill 复制到项目或用户目录
...
| Skill | 作者 | 功能 |
|-------|------|------|
| find-skills | Vercel | 发现其他 Skill |
| web-design-guidelines | Vercel | 100+ 可访问性/UX 规则审查 |
| react-best-practices | Vercel | 57 条 React/Next.js 性能规则 |
| superpowers | obra | 多代理开发工作流 |
| remotion-best-practices | Remotion | 程序化视频开发 |
...
- **Git 版本控制**：通过 Git 跟踪 Skill 的变更历史
- **Skill 级别版本**：自定义 Skill 可使用时间戳或语义化版本号作为标识
...
已知支持或实验性支持 Agent Skills 的平台包括：

- Claude Code / Claude.ai
- Roo Code、Goose、OpenHands 等社区工具
...
所有 Skill 的 description 共享有限的上下文预算。安装过多 Skill 会导致 description 被截断，Claude 失去触发关键词。建议每个项目保持适量的自动触发 Skill，其余设为 `disable-model-invocation: true`。

**YAML 多行 description**：

某些格式化工具（如 Prettier）会将单行 description 折成多行，Claude Code 的 YAML 解析器无法正确处理，导致 Skill 静默消失。保持 description 在单行内，或使用 YAML 双引号包裹：`description: "当用户请求...时使用"`

**Skill 与 CLAUDE.md 的边界**：

- 通用知识（框架 API、项目架构）→ 放入 CLAUDE.md
- 可重复工作流（审查流程、部署步骤）→ 放入 Skill
- 对于通用知识，直接嵌入 CLAUDE.md 通常比 Skill 的按需加载效果更稳定，因为 CLAUDE.md 在每次会话开始时完整加载
...
[^1]: [Claude Code Skills 官方文档](https://code.claude.com/docs/en/skills)
[^2]: [Agent Skills 开放标准](https://agentskills.io)
[^3]: [Anthropic Skills GitHub 仓库](https://github.com/anthropics/skills)
[^4]: [Claude Code Agent SDK Skills 文档](https://code.claude.com/docs/en/agent-sdk/skills)
[^5]: [Inside Claude Code Skills: Structure, prompts, invocation](https://mikhail.io/2025/10/claude-code-skills/) — Mikhail Shilkov
[^6]: [Claude Code Skills and the Agent Skills Open Standard](https://dak-dev.vercel.app/blog/claude-code-skills-agent-skills-standard) — Dakota Smith
[^7]: [Best Claude Code Skills to Try in 2026](https://www.firecrawl.dev/blog/best-claude-code-skills) — Firecrawl