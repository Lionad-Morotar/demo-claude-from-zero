...
| `auto` | 自动批准低风险操作，高风险仍提示 | 熟悉项目后的日常开发 |
| `dontAsk` | 与 `bypassPermissions` 类似，但保留安全阻断 | 仅限完全隔离的临时环境 |

> 注：`auto` 和 `dontAsk` 为部分版本支持的额外模式，具体可用模式以当前版本 `--help` 输出为准。
...
会话中也可以临时切换：按 `Shift+Tab` 循环切换权限模式，或在对话框输入 `/plan` 进入 Plan 模式。

**铁律**：**生产相关项目永远不开 `bypassPermissions`**。哪怕只是"快速跑一下"，下次有人接手这台机器时可能会吃大亏。

### 2.2 .claude/settings.json 的 allow / deny 列表

settings.json 提供了细粒度的工具白名单与黑名单。以下是一份典型的"安全 baseline"配置示例，供参考：

> **免责声明**：官方 settings 文档结构可能随版本更新而变化，以下示例中的 matcher 语法（如 `Bash(pnpm test:*)` 的括号与 glob 规则）基于社区实践整理，未完全确认与官方 schema 的精确对应关系。实际使用前请查阅当前版本的官方文档并做验证。
    ...
    "PreToolUse": [
      {
        ...
        "command": "if find . -maxdepth 2 -type f -size +5M -not -path './node_modules/*' -not -path './.git/*' | grep -q .; then echo 'BLOCKED: 仓库中存在 > 5MB 的文件，请确认是否应该忽略' >&2; exit 1; fi"
      }
    ]
  }
}
...
> **Hook stdin 说明**：官方 Hooks 通过 **stdin** 接收 JSON 数据，而非环境变量。命令钩子中如需读取工具输入，应使用 `jq -r '.tool_input.command' < /dev/stdin`。上例为简化展示，实际复杂场景建议将逻辑封装到外部脚本中处理 stdin。
...
> 外部脚本 `.claude/hooks/check-secret-leak.js` 应从 stdin 读取 JSON 输入：`const input = JSON.parse(require('fs').readFileSync(0, 'utf8'))`。
    ...
    "PreToolUse": [
      {
        ...
        "command": "TARGET=$(jq -r '.tool_input.command // .tool_input.file_path // .tool_input.path // empty' < /dev/stdin); PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || echo \"$PWD\"); if [ -n \"$TARGET\" ] && echo \"$TARGET\" | grep -qE '^~/|^/Users/|^/home/'; then case \"$TARGET\" in \"$PROJECT_ROOT\"/*) ;; *) echo 'BLOCKED: 检测到对项目外路径的操作，请使用 plan 模式确认' >&2; exit 1;; esac; fi"
      }
    ]
  }
}
...
> 上例使用 `git rev-parse --show-toplevel` 获取项目根目录，比 `$(pwd)` 更精确。如果不在 git 仓库中，会回退到 `$PWD`。`jq` 从 stdin 读取官方 JSON 格式，尝试多个可能的字段名（`command`、`file_path`、`path`）以适配不同工具类型。
...
**假设性事故示例**：以下场景为说明风险而构造的假设性示例，非真实事件。某团队工程师为了快速调试一个 Stripe 支付问题，把整段含 `Authorization: Bearer sk_live_xxx` 的失败请求粘贴到 Claude 会话。事后该工程师把会话日志保存到内部知识库分享给同事，知识库被一个新入职但还未完成保密协议培训的实习生看到。事故发现后，团队不得不轮换 Stripe 密钥、重新审计过去 30 天的 API 调用日志、向合规团队报备。**整个事故的"根因"是一次毫无恶意的复制粘贴**——但现代密钥的扩散链路就是这么短、这么不起眼。
...
- 定期运行 `/doctor` 命令检查 Claude Code 健康状态，或编写自定义验证脚本确认 hooks 都活着。
...
[^1]: [Anthropic Claude Code 官方文档](https://code.claude.com/docs/en) — Permission Mode、Hooks、settings.json 全览
[^2]: [Anthropic: Responsible Scaling Policy](https://www.anthropic.com/news/anthropics-responsible-scaling-policy) — 负责任 AI 部署的官方框架
[^3]: [OWASP Top 10](https://owasp.org/www-project-top-ten/) — Web 安全经典威胁清单
[^4]: [OWASP LLM Top 10](https://owasp.org/www-project-top-10-for-large-language-model-applications/) — LLM 应用专属安全威胁清单
[^5]: [Google: Building Secure and Reliable Systems](https://google.github.io/building-secure-and-reliable-systems/) — Google SRE 实践中的安全章节
[^6]: [The Twelve-Factor App — Config](https://12factor.net/zh_cn/config) — 配置与密钥分离的经典原则
[^7]: [Trusted Types W3C 规范](https://www.w3.org/TR/trusted-types/) — 浏览器端的可信类型规范，避免 DOM 注入
[^8]: [Snyk: Supply Chain Security Best Practices](https://snyk.io/series/open-source-security/) — 供应链攻击防御实践
[^9]: [GitHub: Best practices for keeping your supply chain secure](https://docs.github.com/en/code-security/supply-chain-security) — GitHub 官方供应链安全指南
[^10]: [PyPI typo-squatting: Recent attack examples](https://pypi.org/security/) — 真实供应链攻击案例
[^11]: [Charity Majors: Observability Engineering (O'Reilly, 第 21 章)](https://www.oreilly.com/library/view/observability-engineering/9781492076438/) — Honeycomb 创始人对运维成熟度的分层模型
[^12]: [Anthropic Trust & Safety](https://www.anthropic.com/trust) — Anthropic 信任与安全文档中心