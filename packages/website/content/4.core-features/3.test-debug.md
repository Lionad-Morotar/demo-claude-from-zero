...
测试与调试是软件开发的两大支柱，也是 Claude Code[^1] 展现其 Agentic 能力最充分的场景之一。不同于传统 IDE 中孤立的代码补全或静态分析，Claude Code 能够在一个连续的代理循环中完成"编写测试 → 运行测试 → 解析失败 → 定位 Bug → 修复代码 → 验证通过"的完整闭环。本章将深入探讨如何利用 Claude Code 进行高效的测试驱动开发、自动化测试生成、迭代式 Bug 修复以及复杂问题的系统性调试。
...
[^1]: [Claude Code 官方文档 - 常见工作流](https://code.claude.com/docs/en/common-workflows) —— Anthropic 官方测试和调试工作流指南
[^2]: [Claude Code 官方文档 - 测试](https://code.claude.com/docs/en/test-debug) —— 测试生成和调试官方指南
[^3]: [Claude Code 官方文档 - Bash 工具](https://code.claude.com/docs/en/bash) —— 运行命令和测试工具
[^4]: [Claude Code 官方文档 - 调试](https://code.claude.com/docs/en/debug) —— 调试策略和技巧
[^5]: [Test-Driven Development with Claude Code](https://testdriven.io/blog/claude-code-tdd/) —— TDD 实践指南
[^6]: [Claude Code Best Practices - Testing](https://code.claude.com/docs/en/best-practices) —— 测试最佳实践
[^7]: [Effective Debugging with Claude Code](https://www.builder.io/blog/claude-code-debugging) —— 调试技巧
[^8]: [Claude Code for Test Generation](https://www.anthropic.com/engineering/claude-code-testing) —— Anthropic 测试生成博客
[^9]: [Using Claude Code for Legacy Code Testing](https://www.petefreitag.com/blog/claude-code-legacy-testing/) —— 遗留代码测试
[^10]: [Claude Code and pytest](https://pytest-with-eric.com/integrations/claude-code-pytest/) —— pytest 集成
[^11]: [Integration Testing with Claude Code](https://www.testim.io/blog/claude-code-integration-testing/) —— 集成测试