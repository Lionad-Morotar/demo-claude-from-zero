...
- **命令执行**：`Bash`（执行 Shell 命令）、`PowerShell`（Windows 原生 PowerShell 执行）
- **扩展能力**：`Monitor`（通过插件系统启用，用于后台任务监控）
...
> ⚠️ **注意**：Monitor 是 Claude Code 的**插件组件**，需通过插件系统启用，而非内置工具。

Monitor 让 Claude Code 能够事件驱动地响应后台进程，无需阻塞对话（来源：[Claude Code Docs - Plugins Reference](https://code.claude.com/docs/en/plugins-reference)）。
