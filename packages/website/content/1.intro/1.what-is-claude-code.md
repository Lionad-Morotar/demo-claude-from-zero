...
**Claude Code** 是人工智能公司 Anthropic 推出的**代理式编码系统(agentic coding system)**。其**原生形态**是终端 CLI 工具，但 Anthropic 也提供了 VS Code 扩展和 Web 版本作为辅助接入方式。开发者只需在命令行中输入 `claude`，即可启动一个能够理解整个代码库、自主执行多步骤开发任务的 AI 代理。
...
Claude Code 于 **2025 年 2 月 24 日** 作为 research preview 首次公开，随后在 **2025 年 5 月 22 日** 的 Code with Claude 开发者大会上正式发布。它是**推动** AI 辅助编程从代码补全向代理式编码演进的关键产品之一。在此之前，市场上已有 GitHub Copilot（2021 年）、Cursor（2023 年）等工具，但它们的核心能力仍停留在"预测下一行代码"或"在编辑器内提供聊天建议"。Claude Code 的出现，将竞争维度提升到了"自主执行多步骤开发任务"的全新层面。
...
> 以下能力通过 MCP **扩展生态**实现，需用户自行安装和配置对应的 MCP 服务器，非 Claude Code 原生内置。
...
GitHub Copilot 早期以**内联自动补全**为核心，但 2025 年 2 月已推出 **agent mode**（Copilot Edits / Workspace），支持跨文件代理式编辑。Copilot Chat 上下文窗口为 64k-128k token（2024 年 12 月后升级），虽仍低于 Claude Code 的百万级窗口，但已非早期限制。[^5] 两者的核心差异已从"补全 vs 代理"转向"代理框架与工具层的设计质量"。
